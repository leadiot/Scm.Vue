	...
	methods: {
		async open(parent, row) {
			if (!row) {
				...
				this.mode = "edit";
				var res = await this.$API.scmadmregion.model.get(row.id);
			...
			this.$refs.formRef.validate(async (valid) => {
				if (valid) {
					...
					if (this.formData.id === 0) {
						res = await this.$API.scmadmregion.add.post(this.formData);
					} else {
						res = await this.$API.scmadmregion.update.put(this.formData);
					}
