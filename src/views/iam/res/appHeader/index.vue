			...
			<sc-table ref="table" :table-name="tableName" :api-obj="apiObj" :column="column" row-key="id"
				...
				<el-table-column label="操作" align="center" fixed="right" width="210">
					<template #default="scope">
						<el-button link type="primary" size="small" @click="show_info(scope.row)">
							详情
						</el-button>
						<el-divider direction="vertical" />
						<el-button link type="primary" size="small" @click="open_dialog(scope.row)">
							编辑
						</el-button>
						<el-divider direction="vertical" />
				...
				<template #app_code="scope">
					<el-button link type="primary" @click="open_newtab(scope.row)">
						{{ scope.row.app_code }}
					</el-button>
				</template>
...
export default {
	...
	data() {
		return {
			tableName: 'iam_res_app_header',
			apiObj: this.$API.iamresappheader.page,
			param: {
				option_id: '0',
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.iamresappheader.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.iamresappheader.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.iamresappheader.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.iamresappheader.delete, this.selection);
		},
		open_dialog(row) {
		...
		open_newtab(row) {
			...
			this.$router.push({ path: '/iam/res/appdetail', query: { 'id': row.id } });
		},
		show_info(row) {
