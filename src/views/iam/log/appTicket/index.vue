...
export default {
	...
	data() {
		return {
			tableName: 'iam_log_app_ticket',
			apiObj: this.$API.iamlogappticket.page,
			param: {
				option_id: '0',
			...
			column: [
				...
				{ prop: 'request_id', label: '请求', width: 200, align: 'left' },
				{ prop: 'ticket', label: '凭证', minWidth: 160, align: 'left' },
				{ prop: 'handle', label: '作业状态', width: 80 },
				{ prop: 'result', label: '作业结果', width: 80 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.iamlogappticket.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.iamlogappticket.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.iamlogappticket.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.iamlogappticket.delete, this.selection);
		},
		open_dialog(row) {
