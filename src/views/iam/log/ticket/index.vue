...
export default {
	...
	data() {
		return {
			tableName: 'iam_log_osp_ticket',
			apiObj: this.$API.iamlogospticket.page,
			list: [],
			param: {
				option_id: '0',
			...
			column: [
				...
				{ prop: 'update_times', label: '', width: 100 },
				{ prop: 'update_names', label: '', width: 100 },
				{ prop: 'create_times', label: '', width: 100 },
				{ prop: 'create_names', label: '', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.iamlogospticket.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.iamlogospticket.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.iamlogospticket.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.iamlogospticket.delete, this.selection);
		},
		open_dialog(row) {
